...
const Navbar = ({ menuItems = []}) => {
  ...
  return (
    <div className='bg-white w-full pt-[25px] pb-[25px]'>
    <div className="navbar-container" style={{display:"flex",marginLeft:"150px"}}  ref={navRef} onMouseLeave={handleMouseLeave}>
                              ...
                              <ul>
                                {child.children.map((sub, sIndex) => (
                                  <li key={sIndex} className="submenu-item">
                                    <Link to={sub.path} onClick={closeAllMenus}>{sub.label}</Link>
                                  </li>
                                ))}
                              </ul>
                            </div>
